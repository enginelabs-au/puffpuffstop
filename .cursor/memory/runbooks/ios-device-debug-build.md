...
Verified 2026-09-02: Release build 30 installed at `Build/Products/Release-iphoneos/PuffPuffStop.app`. Bump both `app.json` `ios.buildNumber` and `ios/PuffPuffStop/Info.plist` `CFBundleVersion` — Expo does not sync the generated plist. Metro may still start after install; it is not required. Unplug and open the app.
